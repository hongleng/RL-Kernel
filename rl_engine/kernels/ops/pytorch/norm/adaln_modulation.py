...
class _AdaLNReference(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, modulation: torch.Tensor, eps: float):
        ...
        norm = centered * rstd.unsqueeze(-1)
...
class NativeAdaLNModulationOp:
    def __call__(self, x: torch.Tensor, modulation: torch.Tensor, *, eps: float = 1e-6):
        return self.forward(x, modulation, eps=eps)

    def forward(self, x: torch.Tensor, modulation: torch.Tensor, *, eps: float = 1e-6):
